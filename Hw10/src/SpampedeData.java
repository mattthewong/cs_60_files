...
class SpampedeData {
	 ...
	 ************************************/
	//method to access the tail of pede--index 0 of the pedeCells linked list
	
	public MazeCell getPedeTail() {
		return this.pedeCells.get(0);
	}
	
	//if next is not spam
	//public void makeTailEmpty(){
		//pedeCells.removeFirst();
	//}
	
	//if next is spam:
	
	public void advanceHelper(MazeCell nextCell){
		if (nextCell.isSpam()){
			MazeCell currenthead = this.pedeCells.getLast();
			currenthead.becomeBody();
			this.pedeCells.addLast(nextCell);
			nextCell.becomeHead();
			...
			this.pedeCells.addLast(nextCell);
			nextCell.becomeHead();
			MazeCell tail = this.pedeCells.removeFirst();
			tail.becomeOpen();
		}
	...
	public MazeCell getNextCellInDir() {
		if (this.currentMode == 'N'){
			return getNorthNeighbor(this.getPedeHead());
		}
		
		if(this.currentMode == 'E'){
			return getEastNeighbor(this.getPedeHead());
		}
		
		if (this.currentMode == 'W'){
			return getWestNeighbor(this.getPedeHead());
		}
		
		else{
			return getSouthNeighbor(this.getPedeHead());
		}
	}
	...
	public void reversehelper(){
		...
		MazeCell head = this.pedeCells.getLast();
		head.becomeOpen();
		MazeCell newhead = this.pedeCells.getFirst();
		// Step 3: relabel the head
		newhead.becomeHead();
		
		LinkedList<MazeCell> reverselist = new LinkedList<MazeCell>();
		// Step 2: reverse the body parts
		while (this.pedeCells.isEmpty()==false){
			
			reverselist.addLast(pedeCells.removeLast());
		}
		// Step 4: calculate the new direction after reversing!
		this.pedeCells = reverselist;
		
		MazeCell neck = this.getPedeNeck();
		
		 if (neck == this.getCell(newhead.getRow()+1, newhead.getColumn())){
			this.currentMode = 'N';
		}
		 else if (neck == this.getCell(newhead.getRow()-1, newhead.getColumn())){
			 this.currentMode = 'S';
		 }
		 else if (neck == this.getCell(newhead.getRow(), newhead.getColumn()+1)){
			 this.currentMode = 'W';
		 }
		 else{
		 this.currentMode = 'E';
