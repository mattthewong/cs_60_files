...
public class Picture extends SimplePicture 
{
	...
	public Picture lightdarkhelper(int value){
	...
	public Picture addgeneralhelper(int value,int amount){
		...
		for(int x = 0; x < pictureWidth; x++) {
			for(int y = 0; y < pictureHeight; y++) {
				...
				if (value ==1){
				currentPixel.setRed(redComponent + amount);
				currentPixel.setGreen(greenComponent + amount);
				currentPixel.setBlue(blueComponent + amount);
			}
				if (value==2){
					currentPixel.setRed(redComponent - amount);
					currentPixel.setGreen(greenComponent - amount);
	...
	public Picture lighten(int lightenAmount){
		...
		Picture newPicture = new Picture(this);
		Picture X = newPicture.addgeneralhelper(1,lightenAmount);
		return X;		
}
	//////////////////////////// Darken /////////////////////////////////
	/**
	...
	public Picture darken(int darkenAmount) {
		...
		Picture newPicture = new Picture(this);
		Picture X = newPicture.addgeneralhelper(2,darkenAmount);
		return X;
	}
	//////////////////////////// Add[Blue,Green,Red] /////////////////////////////////
	/**
	...
	public Picture addBlue(int amount) {
		...
		Picture newPicture = new Picture(this);
		Picture X = newPicture.addgeneralhelper(3,amount);
		return X;
	}
	...
	public Picture addRed(int amount) {
		...
		Picture newPicture = new Picture(this);
		Picture X = newPicture.addgeneralhelper(4,amount);
		return X;
	}
	...
	public Picture addGreen(int amount) {
		Picture newPicture = new Picture(this);
		Picture X = newPicture.addgeneralhelper(5,amount);
		return X;
	}
	...
	public Picture rotateRight() {
		...
		int pictureheight1 = newpicture.getHeight();
		int picturewidth1 = newpicture.getWidth();
		
		for(int x = 0; x < picturewidth1; x++){
			for(int y=0;y<pictureheight1; y++){
				Pixel newPixel = newpicture.getPixel(x, y);
				Pixel currentPixel = this.getPixel(y, picturewidth1-1-x);
	...
	public Picture luminosity(){
		...
		for(int x = 0; x < pictureWidth; x++) {
			for(int y = 0; y < pictureHeight; y++) {
				...
				int redComponent = newPicture.luminosityOfPixel(x, y);
				int greenComponent = newPicture.luminosityOfPixel(x, y);
				int blueComponent = newPicture.luminosityOfPixel(x, y);
	...
	//used the luminsity function to set picture to luminosity style, then output the
	//luminosity of the desired pixel.
	
		
		Pixel desiredpixel = this.getPixel(x, y);
		Color c = desiredpixel.getColor();
		int redComponent = c.getRed();
		int greenComponent = c.getGreen();
		int blueComponent = c.getBlue();
	...
	public Picture energy(){
		...
		//int pictureHeight = luminositypic.getHeight();
		//int pictureWidth = luminositypic.getWidth();

		//for(int x = 0; x < pictureWidth; x++) {
			//for(int y = 0; y < pictureHeight; y++) {
				//Pixel currentPixel = newPicture.getPixel(x, y);
			//	Color c = currentPixel.getColor();
				//luminositypic.getEnergy(x,y);
				
	//		}
	//	}
	//	return luminositypic;
	//}
	...
	private int getEnergy(int x, int y) {
		...
		int luminosity = luminosityOfPixel(x, y);
		
		
		if (x == pictureWidth-1 && y == pictureHeight-1){
			int horizontal = luminosity-this.luminosityOfPixel(x-1, y);
			int vertical= luminosity-this.luminosityOfPixel(x, y-1);
	 ...
	 */
		...
		public int[] computeSeam(){		
			...
			for(int y=0; y < this.getHeight(); y++)
				for (int x=0; x < this.getWidth(); x++){
					if(y ==0){
						...
						int left = safeArray(y-1, x-1, table);
						int mid = safeArray(y-1, x, table);
						int right = safeArray(y-1, x+1, table);
						min = Math.min(left,Math.min(mid,right));
			...
			int[] myArray = table[(table.length)-1];
			int minIndex = 0;
			int minNum = 10000;
			...
			seam[parent.length-1] = minIndex;
			
			minIndex = minIndex - 1;
			
			for(int a = parent.length-2; a>=0; a--){
				minIndex = parent[a+1][seam[a+1]];
				seam[a] = minIndex;
			}
			for (int b = 0; b < seam.length; b++){
		...
		public int safeArray(int x, int y, int[][] table){
	...
	public Picture carve(){
		...
		for(int y=0; y<newPicture.getHeight(); y++){
			for(int x=0; x<newPicture.getWidth(); x++){
				Pixel currentPixel = newPicture.getPixel(x,y);
				Pixel shiftPixel = this.getPixel(x+1, y);
				Pixel normPixel = this.getPixel(x, y);
				if(x >= seam[y]){
					currentPixel.setRed(shiftPixel.getRed());
					currentPixel.setGreen(shiftPixel.getGreen());
					currentPixel.setBlue(shiftPixel.getBlue());	
					}
				else{
					currentPixel.setRed(normPixel.getRed());
					currentPixel.setGreen(normPixel.getGreen());
					currentPixel.setBlue(normPixel.getBlue());	
				}
		}
		}
		return newPicture;
