...
;; gchildren
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;gets the which takes in
;num ,a nonnegative integer,
;a node, a symbol representing a node in a graph
;a positively-weighted graph
;As output, gchildren should return a list of all of the num-th generation descendants of
;node node in graph graph. If num is 0, then (list node) should be returned, regardless of graph.
;If node node does not have any descendants that are num-levels deep in graph, the empty list should be
;returned. 
...
;provided method to see if a path exists to get from node a to node b in graph G.
...
;additional test cases:
;WRITE THESE CASES 
(check-expect (min-dist 'c 'd graph5) 1)
(check-expect (min-dist 'a 'd graph5) 8)
(check-expect (min-dist 'd 'e graph5) 2)
(check-expect (min-dist 'f 'a graph5) 42000000)
